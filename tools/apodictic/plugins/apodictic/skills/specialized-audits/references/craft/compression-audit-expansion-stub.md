# Compression Audit — Expansion Stub
...
Deliverables:
1) compression-audit-level-setting.md
2) compression-audit.md
...
```markdown
AUDIT NAME: Compression Audit
AUDIT SLUG: compression-audit
AUDIT TYPE: craft
PRIMARY DOMAIN: Content redundancy, over-establishment, expendable material — what should be cut and how much it saves
SCOPE CLASS: cross-genre
RELATED MODULES: Scene Turn (scene-level keep/cut), Short Fiction (compression economy at short-form scale), Pass 2 (orphan scenes, repeated beats), Pass 3 (pacing drag), Pass 1 (reader disengagement)
ACTIVATION TRIGGERS: user-request ("too long," "what can I cut," "needs tightening"), Pass 2 findings (orphan scenes ≥3, proportional imbalance >40%), intake constraint (target word count below current), revision-round re-entry with "cut X words" goal
TARGET DEPTH: 400-550 lines for v1
STATUS: draft
AUTHORING MODE: single-model
```
...
Pass 2 (Structural Mapping) detects orphan scenes ("removable without breaking causality") and repeated beats, and produces word count distribution by chapter, scene, and part. Pass 3 (Rhythm and Modulation) diagnoses pacing drag — scenes that feel slow relative to their structural function. Pass 1 (Reader Experience) flags disengagement, which often correlates with expendable material. The Scene Turn audit (Bickham) tests whether individual scenes produce change, and scenes that don't turn are implicit cut candidates. The Short Fiction audit has a full compression economy framework, but it's scoped to works under ~20,000 words where compression is the *primary* craft concern. The thriller genre module includes a scene compression check for the final quarter. The Franklin Pathway flags pre-complication material that "should be cut, compressed, or repositioned."

### What is missing

> Existing systems diagnose structural problems that *imply* cutting but output their findings as "what's wrong." This audit diagnoses expendable material directly and produces a cut list with estimated word savings — reframing the framework's findings from "fix this" to "remove this."

### Core failure claim

> The key failure is not that the manuscript contains bad writing.
> The key failure is **retained scaffolding**: the manuscript preserves material the writer needed during drafting (establishment, orientation, exploration, redundant emotional beats) that the reader does not need during reading. The draft is carrying its own construction equipment.

### Non-duplication boundary

1. **What this audit does that core passes do not:** Core passes diagnose *why something doesn't work*. This audit identifies *what should be removed* and quantifies the savings. Pass 2 says "these scenes are orphans." This audit says "cut these 3 orphan scenes, these 4 over-established setups, and these 6 redundant emotional beats for ~8,200 words of savings, organized from largest cut to smallest." The output is a prioritized cut list, not a problem inventory.

2. **What this audit does that related specialized audits do not:** Scene Turn diagnoses whether scenes function mechanically — it tests the engine, not whether the car should be in the fleet. Short Fiction's compression economy evaluates ratio of implication to exposition in short form; this audit evaluates expendable material at long-form scale where the problem is different (not "every word must earn its place" but "which of these 300 scenes aren't earning theirs?"). Neither produces a cut list with word savings.

3. **Where it should be run in sequence:** After Pass 2 (needs structural map and word counts) and Pass 1 (needs reader engagement data). Can run concurrently with or after other passes. Most useful when author has a word-count target or when Pass 2 findings suggest proportional imbalance.

### Depth target

1. Enough detail for inter-editor agreement on what constitutes a cut candidate vs. a tighten candidate vs. a keep.
2. Not so broad it re-diagnoses pacing (Pass 3's job) or scene mechanics (Scene Turn's job).
3. Named flags for specific compression failure types, a Cut List artifact as primary output, a Distinguish protocol for "cut entirely" vs. "compress in place" vs. "keep — it's load-bearing," and mode calibration by genre expectations.

---

## Phase 1: Level-Setting Research

Output file: `compression-audit-level-setting.md`

### Research Question 1: Theoretical Grounding

What models explain why manuscripts retain expendable material, and what principles govern successful compression?

Deliverable:
1. 3-6 conceptual frames for why drafts run long and how compression works
2. Operationalized into testable diagnostics

Frames to investigate:
- **Scaffolding theory** (writer-facing vs. reader-facing material — the draft preserves the writer's learning process)
- **Over-establishment** (setup that explains more than the reader needs, driven by writer anxiety about clarity)
- **Diminishing-returns information delivery** (first mention of a detail does work; second mention reinforces; third and beyond produce drag)
- **Scene-level vs. paragraph-level expendability** (wholesale scene removal vs. tightening within scenes — different operations with different risk profiles)
- **Structural load-bearing test** (what happens to causality, character, and theme if this material is removed?)
- **Genre-specific compression norms** (literary fiction tolerates more dwell time than thriller; romance needs more interiority than mystery)

### Research Question 2: Failure Taxonomy

What repeat patterns produce expendable material in long-form manuscripts?

Deliverable:
1. Failure inventory with mechanisms
2. One structural-isomorphism anchor line

Failure categories to inventory:
- **Orphan scenes** (no causal connection to spine — removable without breaking anything)
- **Redundant establishment** (setup that's delivered more than once: the reader is told X, then shown X, then reminded of X)
- **Slow turns** (scenes that take 3,000 words to accomplish what the reader processes in 800)
- **Vestigial subplots** (plot threads that were important in an earlier draft and lost their function)
- **Explanatory material the reader has already inferred** (character spells out what the scene just showed)
- **Transitional scaffolding** (travel scenes, waking-up scenes, getting-to-the-next-scene scenes)
- **Repeated emotional beats** (character processes the same feeling in the same register more than twice without escalation)
- **World-building tourism** (description/explanation that serves the setting but not the story)
- **Throat-clearing openings** (chapters or scenes that start before their dramatic entry point)
- **Extended denouement** (post-climax material that restates the resolution rather than deepening it)

Anchor line: "The draft carries its construction equipment into the finished building."

### Research Question 3: Positive Integration Cases

What does effective compression look like at long-form scale?

Deliverable:
1. Exemplar techniques
2. Cross-case success signals

Investigate:
- How published novels handle establishment (single-pass trust in the reader)
- Late entry / early exit at scene level (in medias res as a scene-level principle, not just an opening strategy)
- Implication over statement (the Short Fiction audit's compression economy, scaled to novel chapters)
- Structural compression (combining two scenes into one that serves both functions)
- Tonal compression (a single image or line that does the work of a paragraph of interiority)

### Research Question 4: Calibration by Mode

How do compression norms shift by genre and form?

Deliverable per mode:
1. Promise (what readers expect regarding pace and density)
2. Common blind spot (where this genre tends to over-establish)
3. False-positive risk (what looks expendable but isn't)

Modes to calibrate:
- Literary fiction (higher tolerance for dwell time; false positive risk: interiority that looks decorative but is load-bearing for voice)
- Thriller/suspense (lowest tolerance; false positive risk: setup that feels slow but is necessary for the payoff)
- Romance (needs emotional beats that look redundant to non-romance readers; false positive risk: interiority and processing scenes that are genre-required)
- SFF (world-building walks a line between tourism and necessary orientation; false positive risk: establishment that feels expendable but prevents confusion later)
- Horror (dwell time can be dread-building; false positive risk: slow scenes that are deliberately creating unease)
- Mystery (information delivery must be precise; false positive risk: red herrings that look like digressions)
- Literary nonfiction/memoir (reflection and digression are structural features; false positive risk: essayistic passages that are the *point*)

### Research Question 5: Distinguish Problem

How to distinguish material that should be cut from material that feels expendable but is structurally load-bearing.

Deliverable:
1. Decision tests
2. 3-way classification outcomes

The core distinguish problem: some material that feels slow or redundant is actually doing structural work the reader won't notice until it's gone. The audit must not produce a cut list that, if followed, breaks the book.

Classification:
1. **Cut** — removal improves the manuscript. No downstream dependency. (Test: remove it mentally; does anything later break or lose impact?)
2. **Compress** — the function is needed but the execution is longer than required. Tighten in place. (Test: can the same function be served in half the words?)
3. **Keep — Load-Bearing** — this material looks expendable but is doing work. (Test: removal would damage causality, character arc, emotional payoff, or reader orientation downstream.)

Decision tests:
- **Causality test:** Does any later scene depend on information, setup, or emotional state established here?
- **First-and-only test:** Is this the only place this information appears? (If yes, cutting it loses the information entirely.)
- **Emotional-escalation test:** Does this beat advance the emotional arc, or does it restate a previous position?
- **Reader-orientation test:** Would a reader be confused or disoriented at a later point if this material were absent?
- **Genre-contract test:** Does the genre promise require this type of material? (Romance readers expect processing scenes; thriller readers expect lean transitions.)

### Research source discipline

1. Prefer primary and durable sources (Bickham on scene mechanics, Browne & King on self-editing, Maass on the breakout novel).
2. Use contemporary craft references where needed (editing guides, developmental editing practice).
3. Separate stable principles (redundancy creates drag) from trend-specific observations (current thriller word-count expectations).
...
Skeleton for `compression-audit.md`.
...
**Core problem:** Manuscripts retain material the writer needed during drafting that the reader does not need during reading. Core passes diagnose problems that imply cutting but frame their findings as "what's wrong." This audit reframes existing and new findings as "what should go" and quantifies the cost of keeping it.

**Fundamental question:** What material can be removed or compressed without damaging causality, character, theme, or reader experience — and how many words would that save?
...
Non-duplication with core passes: Pass 2 identifies structural anomalies; this audit converts them into a removal plan. Pass 3 identifies pacing problems; this audit asks whether the solution is removal rather than revision. Pass 1 identifies disengagement; this audit asks whether the disengaging material is load-bearing or expendable.
...
**Severity ladder** (three levels, consistent across the framework):
...
Visual overview showing which parts of the manuscript carry the most expendable material. Helps the author see where the draft is leanest and where it's carrying the most construction equipment.
...
| **Memoir / Lit Nonfiction** | <5% | >25% | Reflection and digression are structural; highest tolerance. |
...
**Intentional-but-failing rule:** Material can be intentional (the author meant to include it) and still expendable. Intentionality is not an exemption. The test is whether the material does work for the *reader*, not whether the *author* wanted it there.
...
Rate each 0-2 after drafting is complete. Target: 10+ / 12 minimum.

| Criterion | Pass criteria (score 2) | Evidence to cite | Score (0-2) |
|-----------|------------------------|-----------------|-------------|
| **Distinctiveness** | Produces output no existing audit or pass produces. Non-duplication boundary (§1) holds under scrutiny. | Compare Cut List artifact against Pass 2, Scene Turn, Short Fiction outputs. Identify any overlap in deliverable format or diagnostic scope. | ___ |
| **Diagnosability** | Flags are testable and reproducible by a second editor on the same manuscript. Detection criteria are mechanical, not impressionistic. | Run two flags on a test manuscript; verify a second reader would assign the same flag to the same passage. | ___ |
| **Actionability** | Findings imply specific revision scope without ghostwriting. The author knows *what to do* after reading the output. | Review Cut List: does each entry name the material, the reason, the estimated savings, and the downstream check? | ___ |
| **Calibration** | Mode-aware thresholds with explicit tighten/loosen controls. False-positive risk named per mode. | Verify literary fiction and thriller produce different flag severities for the same material type (e.g., SS-4). | ___ |
| **Severity Discipline** | Hard gates are explicit, escalation logic is clear, severity ladder is consistent throughout. | Trace hard gate 4 through the two-pass rule: does it require enumeration? Does it restrict escalation to High-confidence items? | ___ |
| **Integration Fit** | Cleanly composes with APDE workflow. Consumes pass findings; feeds editorial letter; handoff fields defined. | Verify Pass 2 → audit input chain; verify Cut List → §9 appendix placement; verify Pass 11 handoff fields exist. | ___ |
| **Total** | 10+ required to ship | | ___/12 |

If score <10: revise failure taxonomy, tighten artifacts, sharpen non-duplication boundary.
...
Destination: `specialized-audits/references/craft/`

Files:
1. `compression-audit.md` (the audit)
2. `compression-audit-level-setting.md` (optional — include if the level-setting research is substantial enough to be a reference)
...
Activation triggers for SKILL.md description:
- "compression audit," "what to cut," "cut list," "too long," "needs tightening," "word count target," "expendable material," "what can I remove"

Recommended pairings (note in audit and in intake router):
- Scene Turn (scene-level feed)
- Stakes System / Decision Pressure (cross-reference for low-pressure scenes)

### D) Validation pass

Before shipping:
1. Run 2-3 manuscripts through the audit, drawn from different modes (e.g., one literary fiction or memoir, one thriller or mystery, one romance or SFF). A single-manuscript test is insufficient for a cross-genre audit — mode calibration must be validated against actual genre variation.
2. For each pilot: verify Cut List artifact is reproducible on second read of the same manuscript.
3. Verify Distinguish tests produce consistent Cut / Compress / Keep classifications across readers.
4. Verify no contradiction with Pass 2 or Scene Turn findings on the same manuscripts.
5. Verify mode calibration produces observably different results across pilot manuscripts — different flag severities, different density thresholds, different false-positive patterns. If two genre-diverse manuscripts produce identical compression profiles, the calibration isn't working.
6. Verify the two-pass rule (discovery → enumeration) triggers appropriately: at least one pilot should be dense enough to require the enumeration pass.
